 ...
 */
export function showDialog<T>(options: Partial<Dialog.IOptions<T>> = {}): Promise<Dialog.IResult<T>> {
...
export function showErrorMessage(
  title: string,
  error: any,
  buttons: ReadonlyArray<Dialog.IButton> = [Dialog.okButton({ label: 'Dismiss' })],
): Promise<void> {
...
export class Dialog<T> extends Widget {
  ...
  constructor(options: Partial<Dialog.IOptions<T>> = {}) {
    ...
    content.addClass('m-Dialog-content');
  ...
  protected onAfterAttach(msg: Message): void {
    ...
    if (this._focusNodeSelector) {
      let body = this.node.querySelector('.m-Dialog-body');
  ...
  protected _evtClick(event: MouseEvent): void {
    let content = this.node.getElementsByClassName('m-Dialog-content')[0] as HTMLElement;
...
export namespace Dialog {
  ...
  export class Renderer {
    ...
    createHeader(title: Header): Widget {
      ...
      header.addClass('m-Dialog-header');
    ...
    createFooter(buttons: ReadonlyArray<HTMLElement>): Widget {
      ...
      footer.addClass('m-Dialog-footer');
      each(buttons, button => {
    ...
    renderLabel(data: IButton): HTMLElement {
      const e = document.createElement('div');
      e.className = 'm-Dialog-buttonLabel';
  ...
  export const tracker = new WidgetTracker<Dialog<any>>({
    namespace: '@mochi/apputils:Dialog',
  });
}
...
namespace Private {
   ...
   */
  export function handleOptions<T>(options: Partial<Dialog.IOptions<T>> = {}): Dialog.IOptions<T> {
  ...
  export function findFirstFocusable(node: HTMLElement): HTMLElement {
    let candidateSelectors = ['input', 'select', 'a[href]', 'textarea', 'button', '[tabindex]'].join(',');
