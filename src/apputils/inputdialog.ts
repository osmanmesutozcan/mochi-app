...
const INPUT_DIALOG_CLASS = 'm-Input-Dialog';
...
export namespace InputDialog {
   ...
   */
  export function getBoolean(options: IBooleanOptions): Promise<Dialog.IResult<boolean>> {
    return showDialog({
      ...options,
      body: new InputBooleanDialog(options),
      buttons: [Dialog.cancelButton({ label: options.cancelLabel }), Dialog.okButton({ label: options.okLabel })],
...
class InputBooleanDialog extends InputDialogBase<boolean> {
  ...
  constructor(options: InputDialog.IBooleanOptions) {
    ...
    this._input.classList.add('m-mod-styled');
...
class InputItemsDialog extends InputDialogBase<string> {
  ...
  private readonly _list: HTMLSelectElement;
