...
import { find, IIterator, map, some } from '@phosphor/algorithm';
import { AttachedProperty } from '@phosphor/properties';
import { ReactWidget, UseSignal } from './vdom';
import { CommandRegistry } from '@phosphor/commands';
import { ReadonlyJSONObject } from '@phosphor/coreutils';

import { Button, DefaultIconReact } from '@mochi/ui-components';

/**
 * The class name added to toolbars.
 */
const TOOLBAR_CLASS = 'm-Toolbar';

/**
 * The class name added to toolbar items.
 */
const TOOLBAR_ITEM_CLASS = 'm-Toolbar-item';

/**
 * The class name added to toolbar spacer.
 */
const TOOLBAR_SPACER_CLASS = 'm-Toolbar-spacer';
...
export class Toolbar<T extends Widget = Widget> extends Widget {
  ...
  private _insertRelative(at: string, offset: number, name: string, widget: T): boolean {
...
export function ToolbarButtonComponent(props: ToolbarButtonComponent.IProps) {
  ...
  return (
    <Button
      className={props.className ? props.className + ' m-ToolbarButtonComponent' : 'm-ToolbarButtonComponent'}
    ...
    >
      {props.iconClassName && (
        <DefaultIconReact
          name={`${props.iconClassName} m-Icon m-Icon-16`}
          className={'m-ToolbarButtonComponent-icon'}
          fallback={true}
          center={true}
          kind={'toolbarButton'}
          tag={'span'}
      ...
      {props.label && <span className='m-ToolbarButtonComponent-label'>{props.label}</span>}
    </Button>
  );
}
 ...
 */
export function CommandToolbarButtonComponent(props: CommandToolbarButtonComponent.IProps) {
  return (
    <UseSignal
      signal={props.commands.commandChanged}
      shouldUpdate={(sender, args) => (args.id === props.id && args.type === 'changed') || args.type === 'many-changed'}
    >
...
export function addCommandToolbarButtonClass(w: Widget): Widget {
  w.addClass('m-CommandToolbarButton');
  return w;
}
 ...
 */
namespace Private {
  export function propsFromCommand(options: CommandToolbarButtonComponent.IProps): ToolbarButtonComponent.IProps {
