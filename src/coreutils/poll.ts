 ...
 */
const defer = typeof requestAnimationFrame === 'function' ? requestAnimationFrame : setImmediate;

/**
 * A function to cancel a deferred action.
 */
const cancel: (timeout: any) => void =
  typeof cancelAnimationFrame === 'function' ? cancelAnimationFrame : clearImmediate;
...
export class Poll<T = any, U = any, V extends string = 'standby'> implements IObservableDisposable, IPoll<T, U, V> {
   ...
   */
  async schedule(
    next: Partial<IPoll.State<T, U, V> & { cancel: (last: IPoll.State<T, U, V>) => boolean }> = {},
  ): Promise<void> {
    if (this.isDisposed) {
      return;
    }
...
namespace Private {
   ...
   */
  export function sleep(frequency: IPoll.Frequency, last: IPoll.State<any, any, any>): number {
