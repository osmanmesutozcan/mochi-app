...
import { JSONExt, JSONObject, JSONValue, ReadonlyJSONObject, ReadonlyJSONValue } from '@phosphor/coreutils';
...
export class DefaultSchemaValidator implements ISchemaValidator {
   ...
   */
  constructor() {
    // FIXME: Here we have an issue with CSP.
    //  because ajv uses eval to compile these schema templates on
    //  runtime if fails to run properly in Chrome App.

    // this._composer.addSchema(SCHEMA, 'mochi-plugin-schema');
    // this._validator.addSchema(SCHEMA, 'mochi-plugin-schema');
  }
  ...
  validateData(plugin: ISettingRegistry.IPlugin, populate = true): ISchemaValidator.IError[] | null {
    ...
    if (!validate || !compose) {
      if (plugin.schema.type !== 'object') {
        ...
        const message =
          `Setting registry schemas' root-level type must be ` + `'object', rejecting type: ${plugin.schema.type}`;
  ...
  private _addSchema(plugin: string, schema: ISettingRegistry.ISchema): ISchemaValidator.IError[] | null {
    ...
    const validate = validator.getSchema('mochi-plugin-schema');
...
export class SettingRegistry implements ISettingRegistry {
   ...
   */
  async get(plugin: string, key: string): Promise<{ composite: JSONValue; user: JSONValue }> {
  ...
  private async _transform(
    ...
    if (!plugin.schema['mochi.app.transform']) {
      return plugin;
    }
...
namespace Private {
   ...
   */
  export function annotatedDefaults(schema: ISettingRegistry.ISchema, plugin: string): string {
   ...
   * setting data.
   */
  export function annotatedPlugin(plugin: ISettingRegistry.IPlugin, data: JSONObject): string {
   ...
   */
  function defaultDocumentedValue(schema: ISettingRegistry.ISchema, key: string): string {
    ...
    return [prefix(title), prefix(description), defaults].filter(str => str.length).join('\n');
  }

  /**
   * Returns a value-with-documentation-string for a specific schema property.
   */
  function documentedValue(schema: ISettingRegistry.ISchema, key: string, value: JSONValue): string {
    const props = schema.properties && schema.properties[key];
   ...
   */
  export function reifyDefault(schema: ISettingRegistry.IProperty, root?: string): JSONValue | undefined {
