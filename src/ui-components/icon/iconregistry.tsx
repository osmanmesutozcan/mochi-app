...
import { Text } from '@mochi/coreutils';

import { IIconRegistry, Icon } from './interfaces';
import { IconImports } from './iconimports';
import { iconstyle, iconStyleFlat } from '../iconstyle';
...
export class IconRegistry implements IIconRegistry {
  ...
  addIcon(...icons: Icon.IModel[]): void {
    icons.forEach((icon: Icon.IModel) => {
      let className = icon.className ? icon.className : IconRegistry.iconClassName(icon.name);
   ...
   */
  icon(props: Icon.INodeOptions & { container?: HTMLElement }): HTMLElement | null {
    const {
      name,
      className,
      title,
    ...
    if (container && container.dataset.icon && container.dataset.icon === resolvedName && container.children[0]) {
   ...
   */
  iconReact(props: Icon.INodeOptions & { tag?: 'div' | 'span' }): React.ReactElement {
  ...
  resolveSvg(name: string): HTMLElement | null {
    ...
    const svgElement = parser.parseFromString(decodeURI(svgHtml), 'image/svg+xml').documentElement;
  ...
  static iconClassName(name: string): string {
    return 'm-' + Text.camelCase(name, true) + 'Icon';
  }
 ...
 */
export const DefaultIconReact = (props: Icon.INodeOptions & { tag?: 'div' | 'span' }): React.ReactElement => {
